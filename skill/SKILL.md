...
| `hike-add <DAY> <节点名称>` | 在指定日期的安排中新增节点。默认插入在徒步节点之后；可通过 `--before/--after <目标节点>` 指定位置 |
| `hike-remove <DAY> <节点名称>` | 删除指定日期的某个节点（模糊匹配节点名称） |
| `hike-reorder <DAY> <节点名称> <up|down> [步数]` | 调整节点顺序。`up 2` 上移2位，`down 1` 下移1位。也支持 `before|after <目标节点>` |
...
- v0.3.0：行程详情强制完善 + 出行建议/网红推荐 + 装备/待办预填充 + 每日安排补充路线详情 + hike-add/remove/reorder 路线调整命令
- v0.2.1：backlog 修复（delayMinutes/costOverrun/新模式）
- v0.2.0：短信解析 + 实时修订 + 汽车/徒步区分
