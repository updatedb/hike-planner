...
| 原则 | 说明 |
|------|------|
| **只查不买 → 不执行订票** | 不执行订票、下单、候补、支付，但保存行程、目的地、出行时间 |
...
```
hike                                — 纯查询目的地 + 路线（不创建行程）
hike-init [日期] [目的地] [活动]  — 启动规划（目的地必填，未填拒绝）
...
hike-add [dayN] [路线A-B]          — 添加行程段到 DayN
hike-del [dayN]                    — 删除 DayN 整个日程
...
stateDiagram
    [*] --> HIKE: hike [目的地]（纯查询）
    [*] --> INIT: hike-init [日期] [目的地] [活动]
    INIT --> COLLECT: 交互询问需求（含 GPX/KML 提醒）
    COLLECT --> ROUTE: 搜索徒步路线（两步路+小红书+B站）
    COLLECT --> GPX: 用户上传 GPX/KML → 解析+保存+渲染HTML
    ROUTE --> TRANSPORT: 查询大交通（12306/flyai）
    TRANSPORT --> HOTEL: 查询酒店（flyai）
    HOTEL --> CULTURE: 收集人文信息（Wiki+小红书）
    CULTURE --> GENERATE: 生成计划文档
    GENERATE --> MAP: 渲染行程地图（高德+虚线徒步+GPX轨迹）
    MAP --> CONFIRM: 用户审查确认
    CONFIRM --> ACTIVE: 出发（行中）
    ACTIVE --> ACTIVE: hike-today / hike-log
    ACTIVE --> SUMMARY: hike-list <tripId> 归档
    SUMMARY --> [*]
```
...
```
👤 用户: hike-init 2026-06-01 古蜀道 徒步+文化探访

🤖 hike-planner:
  好的！开始规划「古蜀道·徒步+文化探访」的行程。
  📁 计划将保存到 /home/openclaw/travel/trip/
  以下信息还需补充：
  1. 返回日期？（单日可不填）
  2. 目的地在哪个省/市/县？（用于精确地图定位）
  3. 从哪个城市出发？
  4. 如有 GPX/KML 轨迹文件或两步路链接，请提供

👤 用户: 5月16日回，从北京出发。广元市剑阁县。有 GPX 文件。

🤖 hike-planner:
  [两步路] 找到 3 条轨迹，最推荐：小娅子→断碑梁 12.7km
  [GPX] 解析完成，轨迹地图已生成 📎 gpx/古蜀道_轨迹地图.html
  [12306] D2008 成都东07:50→剑门关09:21
  [flyai] 剑门关附近酒店查询...
  
  计划已生成：
  📄 upcoming/gushudao-2026-06/古蜀道·徒步+文化探访出行计划.md
