...
| `hike-config output <路径>` | 设置/切换默认输出目录（持久化到 `{skillDir}/planner/config.json`）。⚠️ 行程数据（含订单信息、GPS 坐标）将保存到此目录，共享服务器环境下其他用户可能可见。 |
| `hike-config baseurl <URL>` | 设置 Web 基础 URL（用于轨迹地图链接，默认 `http://localhost`）。不传参数查看当前值。 |
| `hike-select <行程名>` | 激活/选择某个行程进行管理。匹配 tripId 或目的地关键词（模糊匹配）。选择后 hike-status/today/log 都作用于该行程。 |
| `hike-import <GPX/KML文件路径>` | 导入 GPX/KML 轨迹文件到当前行程。自动提取 GPS 坐标、生成轨迹地图 HTML。⚠️ 轨迹坐标将写入本地 HTML 文件，导入前会弹窗确认。 |
...
```text
hike-import ~/downloads/海坨山环线.gpx
```

> 导入 GPX/KML 轨迹文件到当前选中行程。自动提取 GPS 坐标、计算距离/爬升/下降统计数据、生成交互式轨迹地图 HTML。导入前需用户确认（轨迹坐标将写入本地文件）。
...
> ⚠️ **数据本地化**：行程数据（含订单信息、GPS 坐标、媒体文件）持久化到本地磁盘。在共享服务器环境中，同一服务器的其他用户可能可以查看这些数据。建议使用专用输出目录并设置适当的文件权限。

| 数据类型 | 存储位置 | 风险提示 |
|---------|---------|---------|
| 行程计划 | `outputDir/upcoming/<tripId>/` | 含日期、目的地、交通/酒店订单信息 |
| 轨迹坐标 | `outputDir/upcoming/<tripId>/gpx/` | GPX/KML 精确 GPS 坐标写入 HTML 文件 |
| 媒体文件 | `outputDir/upcoming/<tripId>/media/` | 截图/照片可能含敏感个人信息 |
| 订单短信 | 解析后仅提取关键字段（车次/航班/站点/日期），不保留原始短信全文 |

| 外部服务 | 传输数据 | 用途 |
|---------|---------|------|
| 高德地图 API | 站点名称 / GPS 坐标 | 地理编码、地图链接生成 |
| unpkg.com | 浏览器 IP、轨迹区域 | Leaflet 库加载（轨迹地图 HTML） |
| 高德/OSM 瓦片 | 浏览器 IP、浏览区域 | 地图瓦片加载（轨迹地图 HTML） |

### 安全措施

- **文件类型白名单**：仅允许 `.gpx`/`.kml`（轨迹）和 `.png`/`.jpg`/`.jpeg`/`.gif`/`.webp`/`.bmp`/`.svg`/`.mp4`/`.mov`/`.avi`/`.webm`（媒体）
- **路径范围限制**：文件导入仅允许来自行程输出目录、`~/.openclaw/media/inbound/` 或系统临时目录
- **命令注入防护**：外部脚本调用使用 `execFileSync`（参数数组模式，不经过 shell）
- **操作确认**：轨迹导入、媒体导入、短信解析、输出目录切换均需用户确认
